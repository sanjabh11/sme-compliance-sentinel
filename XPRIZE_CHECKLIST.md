...
- [x] Local repo includes `docs/deployment/cloudrun-render-values.template.json`, `npm run write:cloudrun-values-template -- /secure/local/cloudrun-render-values.json`, `npm run write:cloudrun-release-values -- /secure/local/cloudrun-render-values.json`, and `npm run prepare:cloudrun-render-handoff -- --values /secure/local/cloudrun-render-values.json --out-dir artifacts/deployment --strict` so operators can start from a tracked non-secret template or a Git-prefilled release candidate while keeping filled production values private, surfacing category/business-evidence flags, producing a verified owner packet before values are filled, and failing strict rendering while source revision, hosted URL, OAuth, billing, Gemini key restriction, or Secret Manager version placeholders remain.
