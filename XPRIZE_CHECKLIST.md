...
- [x] Local repo tests verify that Cloud Run credential values use Secret Manager references plus `run.googleapis.com/secrets` lookups, and that raw access tokens, service-account key paths, Workspace refresh tokens, and judge credentials are not committed into the Cloud Run manifest.
