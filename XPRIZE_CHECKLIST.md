...
- [ ] For a one-command handoff folder, run `npm run prepare:local-submission-bundle -- /secure/local/local-submission-bundle` to generate the readiness JSON, summary Markdown, owner packets, packet manifest, manifest-verification JSON, and bundle manifest with SHA-256 hashes, then run `npm run verify:local-submission-bundle -- /secure/local/local-submission-bundle/local-submission-bundle-manifest.json --strict` after transfer or owner edits to detect bundle drift, nested packet-verification failure, missing proof-boundary language, or obvious secret-shaped text. Treat the summary, bundle, packets, and hashes as execution aids only, not proof.
