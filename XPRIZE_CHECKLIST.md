...
- [x] Local repo includes `npm run prepare:cloudrun-dry-run -- --values /secure/local/cloudrun-render-values.json --out-dir artifacts/deployment --release-id RELEASE_ID --strict` to write a private dry-run preflight packet with verifier status, stop conditions, redaction checklist, evidence files, SHA-256 digests for the rendered manifest bundle, and dry-run command before any Cloud Run dry-run.
