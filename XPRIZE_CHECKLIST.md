...
- [ ] Run `/api/production/provisioning`, create a private render-values file from `docs/deployment/cloudrun-render-values.template.json`, render placeholders into an ignored private manifest with `npm run render:cloudrun-manifest -- --values /secure/local/cloudrun-render-values.json --out-dir artifacts/deployment --release-id $SENTINEL_RELEASE_ID --strict`, run `npm run prepare:cloudrun-dry-run -- --values /secure/local/cloudrun-render-values.json --out-dir artifacts/deployment --release-id $SENTINEL_RELEASE_ID --strict`, execute the generated dry-run command only if the packet is `ready-to-dry-run`, then execute the deployment command in the real Google Cloud project.
