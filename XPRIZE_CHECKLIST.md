...
- Release `release-20260527-9366844` is deployed on Cloud Run at `https://sme-workspace-sentinel-298323742260.us-central1.run.app`.
- `artifacts/hosted-proof/release-20260527-9366844/release-evidence-manifest.json` currently verifies three release slots: `cloud-run-deployment`, `live-gemini`, and `deployment-execution-control`.
- The hosted Evidence Vault has checksum-bearing verified artifacts for Cloud Run deployment proof and Gemini usage proof.
- Remaining release slots are still `needs-review`; do not set final XPRIZE proof flags until repository access, human attestation, GCP persistence, Workspace OAuth/sync, cost-control quota evidence, business evidence, demo video, judge access, license/IP, and Devpost submission review are cleared.
...
- [x] Local repo includes `npm run verify:local-submission` to aggregate source-release, provenance, license/IP, Cloud Run deployment-template, judge-access readiness, and business-evidence readiness gates into one local readiness report without treating local output as hosted Gemini, Google Cloud, Workspace, revenue, or user proof.
- [x] Local submission readiness report includes a stop-gated phase plan, evidence-gate progress chart, and owner-routed manual intervention plan for human attestation, Cloud Run render/dry-run, hosted proof capture, and business traction proof, with owner, priority, commands, private artifact paths, proof boundaries, bucket-clean done/pending rows, 1-5 ratings, current-phase remaining percent, overall remaining percent, evidence needed, and no-proof stop conditions.
- [x] Local repo includes `npm run prepare:xprize-attestation -- --out-dir /secure/local/xprize-attestation` to generate a private JSON/Markdown human-review packet for provenance, pre-existing work, license/API/IP, demo, hosted proof, business-evidence, eligibility, judge-access, OAuth-scope, and Gemini quota flags without setting or approving any `XPRIZE_*` flag.
...
- [x] Category selected: Small Business Services.
- [x] App uses at least one Google Cloud product in production; release `release-20260527-9366844` has verified Cloud Run deployment proof in the hosted release manifest.
- [x] Deployed LLM functionality includes at least one Gemini API call; release `release-20260527-9366844` has verified hosted `provider=gemini-api` proof in the hosted release manifest.
...
- [x] Local repo includes `npm run verify:judge-access -- --out /secure/local/judge-access-readiness.json` to persist a private judge-access readiness packet and fail strict mode while hosted access, repository/source access, demo clearance, private testing instructions, free judging-period access, or two-business-day response ownership are missing.
...
- [x] Production Launch Command Center surfaces source completeness, submission close timestamp, working project access, testing instructions, evidence-response SLA, and private evidence-response contact from runtime config so dashboard readiness cannot bypass the Cloud Run deployment contract.
- [x] Local repo includes `npm run verify:production -- --url https://YOUR-CLOUD-RUN-URL --release-id $SENTINEL_RELEASE_ID --strict --out /secure/local/hosted-proof/$SENTINEL_RELEASE_ID/verify-production-readonly.json` and the write-through variant with `--include-write-checks --out /secure/local/hosted-proof/$SENTINEL_RELEASE_ID/verify-production-write.json` for hosted readiness smoke reports, with a `releaseLineage` gate that blocks stale release/product URL/provenance evidence, and a structured `hosted-proof-capture` blocker with engineering owner, commands, private artifact paths, proof boundary, and stop conditions when the hosted URL is missing.
- [x] Local repo includes `npm run collect:hosted-proof -- --url https://YOUR-CLOUD-RUN-URL --release-id RELEASE_ID --include-write-checks --strict` to capture an ignored local bundle of redacted hosted proof JSON for deployment, judge access, source release, license/IP review, Devpost, demo-video, Workspace sync status, evidence-intake, Claim Guard, and production verification surfaces.
- [x] Local hosted proof collector emits `release-evidence-manifest.json` so Cloud Run, production readiness, live Gemini, GCP persistence, Workspace renewal, cost controls, business proof, judge access, deployment execution control, demo video, repository, license/IP, and Devpost claim-safety slots stay grouped by release without treating missing or mock-only proof as complete.
...
- [x] Local repo includes `npm run import:hosted-proof -- --bundle-dir artifacts/hosted-proof/RELEASE_ID --url https://YOUR-CLOUD-RUN-URL --dry-run|--confirm-import` plus `npm run prepare:deployment-execution-checklist -- --bundle-dir artifacts/hosted-proof/RELEASE_ID --write-results-template /secure/local/deployment-command-results.json` and `--results /secure/local/deployment-command-results.json --strict` so confirmed imports post only redacted hosted `verify-production.json`, require a passed command-result gate, and never put admin tokens on the command line.
- [x] Local hosted proof import rejects confirmed Evidence Vault writes unless `deployment-execution-checklist.json` contains passed command-results template lineage for the same release id, hosted URL, and required command count.
- [x] Local repo includes `docs/deployment/cloudrun-render-values.template.json`, `npm run write:cloudrun-values-template -- /secure/local/cloudrun-render-values.json`, `npm run write:cloudrun-release-values -- /secure/local/cloudrun-render-values.json`, `npm run prepare:cloudrun-render-handoff -- --values /secure/local/cloudrun-render-values.json --out-dir artifacts/deployment --strict`, and `npm run verify:cloudrun-render-handoff -- artifacts/deployment/$SENTINEL_RELEASE_ID/cloudrun-render-handoff.json --strict` so operators can start from a tracked non-secret template or a Git-prefilled release candidate while keeping filled production values private, surfacing category/business-evidence flags, producing and verifying a handoff plus owner packet before values are filled, and failing strict rendering while source revision, hosted URL, OAuth, billing, Gemini key restriction, or Secret Manager version placeholders remain.
- [x] Local repo includes `npm run audit:cloudrun-values -- --values /secure/local/cloudrun-render-values.json --out-dir artifacts/deployment --release-id RELEASE_ID --strict` to write a private render-values audit packet plus `cloudrun-render-evidence-packet.json/.md` before rendering, with missing-value stops, release-id consistency checks, stale source/URL/resource checks, submission-close and evidence-response SLA checks, derived value review, Secret Manager version checks, manual evidence flags, redaction guidance, and owner/category/status queues for every private value with accepted proof, private-handling rule, dry-run blocker status, public-claim evidence queue, and exact fix. `npm run verify:cloudrun-render-evidence -- artifacts/deployment/RELEASE_ID/cloudrun-render-evidence-packet.json --strict` then writes `cloudrun-render-evidence-packet-verifier.json` and detects packet drift, Markdown edits, missing command/stop-condition gates, audit mismatch, or obvious secret-shaped text after handoff.
- [x] Local Cloud Run contract records repository access mode, required private-repository judge/testing emails, and judging-period end metadata so repository and judge-access claims cannot be represented by bare booleans alone.
- [x] Local Cloud Run verifier blocks contradictory XPRIZE proof flag combinations, including business-model evidence without revenue/cost/user proof, category impact without business/user proof, AI-native operations without Google Cloud/Gemini/product/agent-log proof, and evidence-response readiness without repository, judge-access, demo-clearance, third-party, and IP-review flags.
...
- [x] Local repo includes `npm run prepare:cloudrun-dry-run -- --values /secure/local/cloudrun-render-values.json --out-dir artifacts/deployment --release-id RELEASE_ID --strict` and `npm run verify:cloudrun-dry-run-packet -- artifacts/deployment/RELEASE_ID/cloudrun-dry-run-preflight-packet.json --strict` to write a private dry-run preflight packet, automatically write a `cloudrun-dry-run-packet-verifier.json` proof artifact beside it, hash the rendered manifest bundle, block dry-run if any rendered artifact drifts before `gcloud`, and preserve the local phase rating plus next external-proof handoff for private dry-run/deploy/describe transcript artifacts.
- [x] Local Cloud Run deployment transcript collector checks real `gcloud run services describe` output against the deployment contract, required Secret Manager env refs, `SENTINEL_RELEASE_ID`, release-bound image, dedicated runtime service account, deployed static-egress VPC connector and `all-traffic` annotations, service URL, revision, redacted dry-run/deploy logs, and fatal-output markers before hosted verification can be treated as the next step.
- [x] Local repo includes `cloudrun.service.yaml` with production-mode placeholders for a release-id-tagged Cloud Run image, source revision metadata, Firestore, BigQuery, Pub/Sub, OAuth client and least-privilege scope review metadata, explicit Drive webhook URL, Workspace webhook authentication, Sensitive Data Protection, Gemini guardrails, Cloud Run static-egress VPC connector routing, public judge-testable ingress, bounded max scale, explicit concurrency/timeout/port/CPU/memory settings, XPRIZE category, source/repository-access, submission deadline, working project access, testing instructions, evidence-response SLA/contact, Google Cloud product, deployed Gemini API-call, revenue/cost/CAC/user/product-running evidence attestations, judge/demo flags, Cloud Billing controls, and Cloud Run YAML Secret Manager lookup annotations.
- [x] Local repo includes `docs/deployment/cloudrun-deployment-contract.json`; the TypeScript verifier, CLI verifier, and manifest renderer read required Cloud Run service shape, env flags, Secret Manager references, manual-review flags, evidence-flag dependencies, and prohibited credential env names from that contract, with tests cross-checking the manifest and private render-values template.
...
- [x] Local repo tests verify that Cloud Run credential values use numeric Secret Manager versions plus `run.googleapis.com/secrets` lookups, that business-model, category-impact, AI-native operations, OAuth scope review, IP ownership, and evidence-response flags remain manual-review only, and that duplicate env names, raw secret-shaped env values, access tokens, service-account key paths, Workspace refresh tokens, judge credentials, invalid service shape, invalid production-mode flags, invalid static-egress annotations, invalid entrant types, invalid demo-video hosts, malformed OAuth client ids, Drive webhook URL drift, restricted first-pilot OAuth scopes, missing deferred restricted Drive scope documentation, wildcard Gemini IP allowlists, and non-positive Gemini budget/quota/cost values are blocked before Cloud Run dry-run.
- [x] Local repo tests keep `.env.example`, `cloudrun.service.yaml`, and `docs/deployment/cloudrun-deployment-contract.json` aligned across XPRIZE repository/source proof, submission deadline, working project access, testing instructions, business evidence, demo video, judge access, two-business-day evidence-response handling, Google Cloud persistence, Workspace OAuth scopes/sync, Gemini, cost-control, Secret Manager, and IP/license review flags without committing real credentials.
- [x] Local Cloud Run verifier blocks mutable `latest` container tags and rendered image tags that do not match `SENTINEL_RELEASE_ID`; the checked-in template now uses `RELEASE_ID` as the image-tag placeholder rather than `latest`.
...
- [x] Local MVP exposes `/api/production/deployment-packet` to bind release id, Cloud Run manifest state, render-values audit, render-evidence packet verifier, hosted verification commands, hosted proof bundle artifacts, command-results template artifact, deployment execution checklist artifact, source/provenance checks, redaction rules, private storage paths, and Evidence Vault import template into one private judge-packet plan.
- [x] Local MVP exposes a stop-gated private deployment runbook inside `/api/production/deployment-packet`, mapping each command phase to exact proof files, redaction checks, stop conditions, release-integrity checks, hosted proof import dry-run/confirm commands, and external-proof boundaries.
...
- [x] Repository includes necessary source code; `npm run verify:source-release` reports `published` with no unpublished changes and no obvious secret or unsafe public-claim findings.
...
- [x] Keep OAuth scopes minimal and documented in the app, Cloud Run manifest, deployment contract, private render-values template, and verifier.
...
- [x] Local MVP exposes Evidence Copilot capabilities for redacted source-grounded search, cited synthesis packs, allowlisted evidence metrics/Text-to-SQL planning, and local multimodal document-intake analysis without treating local output as hosted, revenue, user, judge, certification, BigQuery live-query, Gemini File Search, or Document AI proof.
- [x] Local Evidence Copilot keeps Gemini File Search, live BigQuery execution, and Document AI Layout Parser as adapter boundaries until each production service is separately configured, consented, deployed, and verified.
...
- [x] Restrict the production Gemini API key to the required Google API target and server-only client boundary; hosted cost-control write-through confirms the key is restricted to `generativelanguage.googleapis.com` and one configured server IP.
- [ ] Capture private proof of Gemini quota/usage controls before public launch.
- [ ] After repository access, Cloud Run/GCP proof, and hosted Gemini API-call proof are verified, set `XPRIZE_REPOSITORY_ACCESS_CONFIGURED=true`, `XPRIZE_GOOGLE_CLOUD_PRODUCT_EVIDENCE_CONFIGURED=true`, and `XPRIZE_GEMINI_API_CALL_EVIDENCE_CONFIGURED=true` only in private production deployment values; if the source repo is private-shared, verify `XPRIZE_REPOSITORY_JUDGE_ACCESS_EMAILS` includes the required judge/testing accounts.
- [ ] Run `/api/production/provisioning`, create a private render-values handoff with `npm run prepare:cloudrun-render-handoff -- --values /secure/local/cloudrun-render-values.json --out-dir artifacts/deployment --strict` so source/release metadata is prefilled from the pushed commit and the non-strict owner packet is verified before private values are filled, verify the handoff after transfer or owner edits with `npm run verify:cloudrun-render-handoff -- artifacts/deployment/$SENTINEL_RELEASE_ID/cloudrun-render-handoff.json --strict`, fill the remaining non-secret production values privately, audit it with `npm run audit:cloudrun-values -- --values /secure/local/cloudrun-render-values.json --out-dir artifacts/deployment --release-id $SENTINEL_RELEASE_ID --strict`, verify the owner packet with `npm run verify:cloudrun-render-evidence -- artifacts/deployment/$SENTINEL_RELEASE_ID/cloudrun-render-evidence-packet.json --strict`, render placeholders into an ignored private manifest with `npm run render:cloudrun-manifest -- --values /secure/local/cloudrun-render-values.json --out-dir artifacts/deployment --release-id $SENTINEL_RELEASE_ID --strict`, run `npm run prepare:cloudrun-dry-run -- --values /secure/local/cloudrun-render-values.json --out-dir artifacts/deployment --release-id $SENTINEL_RELEASE_ID --strict`, run `npm run verify:cloudrun-dry-run-packet -- artifacts/deployment/$SENTINEL_RELEASE_ID/cloudrun-dry-run-preflight-packet.json --strict` so `cloudrun-dry-run-packet-verifier.json` is written beside the packet, execute the generated dry-run command only if both packet verifiers are `verified`, then execute the deployment command in the real Google Cloud project.
- [x] Run `npm run verify:local-submission -- --out /secure/local/local-submission-readiness.json` before each external evidence collection pass, and follow the emitted phase plan so any `blocked`, `warning`, or `external-required` item becomes a named owner/action instead of an open-ended strategy loop. Run `npm run prepare:submission-summary -- /secure/local/local-submission-summary.md` for a concise private Markdown progress chart with gate, phase, owner, blocker, rating, remaining-percent, and stop-condition tables. Run `npm run prepare:manual-intervention -- /secure/local/manual-intervention-packets` to generate private Markdown packets plus a SHA-256 manifest for engineering, founder/legal, and founder/sales, then run `npm run verify:manual-intervention -- /secure/local/manual-intervention-packets/manual-intervention-manifest.json --strict` after owner review to detect packet drift or secret-shaped text. Latest private packet set for release `release-20260527-9366844` verified with 28 passed checks and 0 blockers.
- [x] For a one-command handoff folder, run `npm run prepare:local-submission-bundle -- /secure/local/local-submission-bundle` to generate the readiness JSON, summary Markdown, owner packets, packet manifest, manifest-verification JSON, and bundle manifest with SHA-256 hashes, then run `npm run verify:local-submission-bundle -- /secure/local/local-submission-bundle/local-submission-bundle-manifest.json --strict` after transfer or owner edits to detect bundle drift, nested packet-verification failure, missing proof-boundary language, or obvious secret-shaped text. Latest private bundle for release `release-20260527-9366844` verified with 58 passed checks and 0 blockers. Treat the summary, bundle, packets, and hashes as execution aids only, not proof.
- [ ] Run `npm run verify:business-evidence -- --write-template /secure/local/business-evidence-template.json --out /secure/local/business-evidence-readiness.json`, fill the private template from redacted invoices/payment exports/user logs/cost-CAC proof, then rerun with `--evidence /secure/local/business-evidence.json --strict` before setting business-evidence flags.
- [ ] Run `npm run prepare:xprize-attestation -- --out-dir /secure/local/xprize-attestation`, review both files privately, and only then decide which `XPRIZE_*` flags can be set from verified private evidence.
...
- [x] Run `npm run collect:cloudrun-deployment -- --release-id $SENTINEL_RELEASE_ID --dry-run-log /secure/local/cloudrun/$SENTINEL_RELEASE_ID/cloudrun-dry-run.log --deploy-log /secure/local/cloudrun/$SENTINEL_RELEASE_ID/cloudrun-deploy.log --describe-json /secure/local/cloudrun/$SENTINEL_RELEASE_ID/cloudrun-describe.json --out-dir artifacts/deployment --strict` after real Cloud Run dry-run/deploy/describe and preserve the redacted checksummed transcript packet in the private judge evidence store.
- [x] Create `sentinel-admin-action-token` and private evidence storage before importing hosted verification proof from the deployed app.
- [x] Run hosted write-through proof commands from a private shell with `SENTINEL_ADMIN_ACTION_TOKEN` configured; do not place the token in source, screenshots, or public Devpost fields.
- [x] Run `POST /api/production/gemini-smoke` from the hosted Cloud Run URL after configuring the Gemini API key, then persist the resulting `provider=gemini-api` agent-run row to BigQuery.
...
- [x] Run `/api/production/hosted-evidence` from the hosted app and attach the resulting capture packet to the private judge binder after the Cloud Run and Gemini artifacts are redacted and checksummed. Remaining artifacts still need review before final judge use.
...
- [ ] Run `npm run verify:judge-access -- --out /secure/local/judge-access-readiness.json --strict` after hosted deployment, demo upload, and private testing instructions are ready; preserve the JSON in the private judge packet and keep credentials out of source.
- [x] Run `npm run verify:production -- --url $NEXT_PUBLIC_PRODUCT_URL --release-id $SENTINEL_RELEASE_ID --strict --out /secure/local/hosted-proof/$SENTINEL_RELEASE_ID/verify-production-readonly.json`, then run `npm run verify:production -- --url $NEXT_PUBLIC_PRODUCT_URL --release-id $SENTINEL_RELEASE_ID --strict --include-write-checks --out /secure/local/hosted-proof/$SENTINEL_RELEASE_ID/verify-production-write.json` after Cloud Run/GCP credentials are configured and attach the JSON outputs to the private judge packet. Release `release-20260527-9366844` still reports `needs-review` overall because external proof gates remain open.
- [x] Run `npm run collect:hosted-proof -- --url $NEXT_PUBLIC_PRODUCT_URL --release-id $SENTINEL_RELEASE_ID --include-write-checks --strict` from a private operator shell after credentials are configured; review the generated `artifacts/hosted-proof/` bundle, release-integrity status, and `release-evidence-manifest.json` for redaction and missing-proof status before judge sharing.
- [x] Run `npm run import:hosted-proof -- --bundle-dir artifacts/hosted-proof/$SENTINEL_RELEASE_ID --url $NEXT_PUBLIC_PRODUCT_URL --dry-run` and review `evidence-vault-import-request.json`, then run `npm run prepare:deployment-execution-checklist -- --bundle-dir artifacts/hosted-proof/$SENTINEL_RELEASE_ID --write-results-template /secure/local/deployment-command-results.json`, fill the private results template from reviewed command output, and run `npm run prepare:deployment-execution-checklist -- --bundle-dir artifacts/hosted-proof/$SENTINEL_RELEASE_ID --results /secure/local/deployment-command-results.json --strict` so the private bundle has a passed command-result gate before making the hosted Evidence Vault write; every command result must match the release id and hosted URL, point at the expected private artifact path, include a SHA-256 checksum, and preserve the generated results-template lineage.
- [x] Run `npm run import:hosted-proof -- --bundle-dir artifacts/hosted-proof/$SENTINEL_RELEASE_ID --url $NEXT_PUBLIC_PRODUCT_URL --confirm-import` from a private operator shell with `SENTINEL_ADMIN_ACTION_TOKEN` set, then confirm Cloud Run and Gemini artifacts have SHA-256 checksums in the Evidence Vault. GCP persistence, Workspace, Cloud Billing quota evidence, repository/human attestation, readiness, and business artifacts remain needs-review.
