...
- [x] Local repo includes `npm run audit:cloudrun-values -- --values /secure/local/cloudrun-render-values.json --out-dir artifacts/deployment --release-id RELEASE_ID --strict` to write a private render-values audit packet before rendering, with missing-value stops, derived value review, Secret Manager version checks, manual evidence flags, and redaction guidance.
