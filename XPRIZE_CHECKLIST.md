...
- [x] Local repo includes `npm run audit:cloudrun-values -- --values /secure/local/cloudrun-render-values.json --out-dir artifacts/deployment --release-id RELEASE_ID --strict` to write a private render-values audit packet before rendering, with missing-value stops, release-id consistency checks, stale source/URL/resource checks, derived value review, Secret Manager version checks, manual evidence flags, and redaction guidance.
...
- [x] Local repo tests keep `.env.example`, `cloudrun.service.yaml`, and `docs/deployment/cloudrun-deployment-contract.json` aligned across XPRIZE repository/source proof, business evidence, demo video, judge access, Google Cloud persistence, Workspace OAuth scopes/sync, Gemini, cost-control, Secret Manager, IP/license review, and evidence-response flags without committing real credentials.
