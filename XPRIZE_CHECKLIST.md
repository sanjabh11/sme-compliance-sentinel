...
- [ ] Run `/api/production/provisioning`, create a private render-values file with `npm run write:cloudrun-release-values -- /secure/local/cloudrun-render-values.json` so source/release metadata is prefilled from the pushed commit, fill the remaining non-secret production values privately, audit it with `npm run audit:cloudrun-values -- --values /secure/local/cloudrun-render-values.json --out-dir artifacts/deployment --release-id $SENTINEL_RELEASE_ID --strict`, render placeholders into an ignored private manifest with `npm run render:cloudrun-manifest -- --values /secure/local/cloudrun-render-values.json --out-dir artifacts/deployment --release-id $SENTINEL_RELEASE_ID --strict`, run `npm run prepare:cloudrun-dry-run -- --values /secure/local/cloudrun-render-values.json --out-dir artifacts/deployment --release-id $SENTINEL_RELEASE_ID --strict`, run `npm run verify:cloudrun-dry-run-packet -- artifacts/deployment/$SENTINEL_RELEASE_ID/cloudrun-dry-run-preflight-packet.json --strict` so `cloudrun-dry-run-packet-verifier.json` is written beside the packet, execute the generated dry-run command only if the packet verifier is `verified`, then execute the deployment command in the real Google Cloud project.
