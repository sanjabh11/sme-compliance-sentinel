...
- [x] Local repo includes `npm run import:hosted-proof -- --bundle-dir artifacts/hosted-proof/RELEASE_ID --url https://YOUR-CLOUD-RUN-URL --dry-run|--confirm-import` plus `npm run prepare:deployment-execution-checklist -- --bundle-dir artifacts/hosted-proof/RELEASE_ID --results /secure/local/deployment-command-results.json --strict` so confirmed imports post only redacted hosted `verify-production.json`, require a passed command-result gate, and never put admin tokens on the command line.
...
- [ ] Run `npm run import:hosted-proof -- --bundle-dir artifacts/hosted-proof/$SENTINEL_RELEASE_ID --url $NEXT_PUBLIC_PRODUCT_URL --dry-run` and review `evidence-vault-import-request.json`, then run `npm run prepare:deployment-execution-checklist -- --bundle-dir artifacts/hosted-proof/$SENTINEL_RELEASE_ID --results /secure/local/deployment-command-results.json --strict` so the private bundle has a passed command-result gate before making the hosted Evidence Vault write; every command result must match the release id and hosted URL, point at the expected private artifact path, and include a SHA-256 checksum.
