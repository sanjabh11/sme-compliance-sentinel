...
- [x] Local MVP exposes `/api/production/provisioning` with non-secret Google Cloud setup commands, API enablement, service-account/IAM sequence, Secret Manager secret names, static-egress Cloud NAT/VPC connector setup, render-values audit command, Cloud Run dry-run preflight and packet-verifier commands, Cloud Run dry-run/deploy commands, redacted deployment transcript collector, release-bound hosted proof collection/import commands, deployment execution checklist gate, verification sequence, and private-handling rules.
