...
- [ ] Run `npm run verify:local-submission -- --out /secure/local/local-submission-readiness.json` before each external evidence collection pass, and treat any `blocked` or `warning` gate as a named owner/action instead of continuing an open-ended strategy loop.
