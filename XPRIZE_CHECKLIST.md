...
- [ ] Run `npm run verify:local-submission -- --out /secure/local/local-submission-readiness.json` before each external evidence collection pass, and follow the emitted phase plan so any `blocked`, `warning`, or `external-required` item becomes a named owner/action instead of an open-ended strategy loop.
