...
- [x] Local repo includes `npm run prepare:cloudrun-dry-run -- --values /secure/local/cloudrun-render-values.json --out-dir artifacts/deployment --release-id RELEASE_ID --strict` and `npm run verify:cloudrun-dry-run-packet -- artifacts/deployment/RELEASE_ID/cloudrun-dry-run-preflight-packet.json --strict` to write a private dry-run preflight packet, hash the rendered manifest bundle, and block dry-run if any rendered artifact drifts before `gcloud`.
...
- [ ] Run `/api/production/provisioning`, create a private render-values file from `docs/deployment/cloudrun-render-values.template.json`, audit it with `npm run audit:cloudrun-values -- --values /secure/local/cloudrun-render-values.json --out-dir artifacts/deployment --release-id $SENTINEL_RELEASE_ID --strict`, render placeholders into an ignored private manifest with `npm run render:cloudrun-manifest -- --values /secure/local/cloudrun-render-values.json --out-dir artifacts/deployment --release-id $SENTINEL_RELEASE_ID --strict`, run `npm run prepare:cloudrun-dry-run -- --values /secure/local/cloudrun-render-values.json --out-dir artifacts/deployment --release-id $SENTINEL_RELEASE_ID --strict`, execute the generated dry-run command only if the packet is `ready-to-dry-run`, then execute the deployment command in the real Google Cloud project.
