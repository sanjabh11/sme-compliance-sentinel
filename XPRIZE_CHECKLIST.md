...
- [x] Local repo includes `npm run verify:production -- --url https://YOUR-CLOUD-RUN-URL --release-id $SENTINEL_RELEASE_ID --out /secure/local/verify-production.json` for hosted readiness smoke reports, with optional write-through checks after production credentials are ready, a `releaseLineage` gate that blocks stale release/product URL/provenance evidence, and a structured `hosted-proof-capture` blocker with engineering owner, commands, private artifact paths, proof boundary, and stop conditions when the hosted URL is missing.
