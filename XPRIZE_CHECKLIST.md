...
- [x] Local repo includes `npm run audit:cloudrun-values -- --values /secure/local/cloudrun-render-values.json --out-dir artifacts/deployment --release-id RELEASE_ID --strict` to write a private render-values audit packet before rendering, with missing-value stops, release-id consistency checks, stale source/URL/resource checks, submission-close and evidence-response SLA checks, derived value review, Secret Manager version checks, manual evidence flags, redaction guidance, and an owner/category/status intake matrix for every private value with accepted proof, private-handling rule, and exact fix.
