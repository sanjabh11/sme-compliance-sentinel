...
describe("package verification scripts", () => {
  it("keeps standard verification commands on stable local runner paths", () => {
    ...
    expect(packageJson.scripts["verify:gemini-model"]).toBe("node scripts/verify-gemini-model-readiness.mjs");
    expect(packageJson.scripts["verify:hosted-lockdown"]).toBe("node scripts/verify-hosted-lockdown.mjs");
