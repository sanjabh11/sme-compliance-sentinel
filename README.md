...
27. Run **Cloud Run evidence** or `npm run verify:cloudrun-deployment` to confirm the checked-in manifest still has only template replacement gaps, Secret Manager references, and manual attestation flags. Start from `docs/deployment/cloudrun-render-values.template.json` or run `npm run write:cloudrun-values-template -- /secure/local/cloudrun-render-values.json`, fill the private copy with production values, then run `npm run audit:cloudrun-values -- --values /secure/local/cloudrun-render-values.json --out-dir artifacts/deployment --release-id $SENTINEL_RELEASE_ID --strict` before rendering. Only after the audit is `ready-to-render`, run `npm run render:cloudrun-manifest -- --values /secure/local/cloudrun-render-values.json --out-dir artifacts/deployment --release-id $SENTINEL_RELEASE_ID --strict` before a real dry-run.
