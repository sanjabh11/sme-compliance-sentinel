...
- Evidence Vault proof importer that converts redacted hosted verification JSON into checksummed Cloud Run, GCP persistence, Gemini, Workspace, Cloud Billing, repository, and readiness artifact records without storing raw tokens in the dashboard response, and rejects bundles whose claimed XPRIZE proof flags lack matching hosted evidence.
...
- Production Provisioning Pack that generates non-secret Google Cloud setup commands, API enablement, service-account/IAM steps, Secret Manager names, render-values audit, Cloud Run dry-run/deploy commands, verification sequence, and private-handling rules.
...
- Private Cloud Run Render-Values Audit that checks a filled non-secret values file for missing strict keys, placeholder values, derived deployment identifiers, Secret Manager version references, manual evidence flags, and redaction risks before any manifest render.
...
26. Run **Provisioning pack** to generate the non-secret Cloud Run/GCP setup sequence, Secret Manager checklist, render-values audit step, dry-run command, deploy command, and hosted verification sequence.
27. Run **Cloud Run evidence** or `npm run verify:cloudrun-deployment` to confirm the checked-in manifest still has only template replacement gaps, Secret Manager references, and manual attestation flags. Start from `docs/deployment/cloudrun-render-values.template.json` or run `npm run write:cloudrun-values-template -- /secure/local/cloudrun-render-values.json`, fill the private copy with production values, then run `npm run audit:cloudrun-values -- --values /secure/local/cloudrun-render-values.json --out-dir artifacts/deployment --release-id $SENTINEL_RELEASE_ID --strict` before rendering. The audit blocks missing placeholders, release-id drift, stale source metadata, HTTP/local product URLs, invalid demo hosts, category drift, mismatched OAuth/Pub/Sub/billing/Gemini resources, invalid Gemini IP allowlists, and nonnumeric Secret Manager versions. Only after the audit is `ready-to-render`, run `npm run render:cloudrun-manifest -- --values /secure/local/cloudrun-render-values.json --out-dir artifacts/deployment --release-id $SENTINEL_RELEASE_ID --strict` before a real dry-run.
...
30. Run `npm run collect:hosted-proof -- --url $NEXT_PUBLIC_PRODUCT_URL --release-id $SENTINEL_RELEASE_ID` after deployment to capture an ignored local bundle of redacted hosted proof JSON, `release-evidence-manifest.json`, and release-integrity checks. Then run `npm run import:hosted-proof -- --bundle-dir artifacts/hosted-proof/$SENTINEL_RELEASE_ID --url $NEXT_PUBLIC_PRODUCT_URL --dry-run` to preview the Evidence Vault import request, followed by `npm run prepare:deployment-execution-checklist -- --bundle-dir artifacts/hosted-proof/$SENTINEL_RELEASE_ID --write-results-template /secure/local/deployment-command-results.json` to generate the private command-results template. Fill that private template from reviewed command output, then run `npm run prepare:deployment-execution-checklist -- --bundle-dir artifacts/hosted-proof/$SENTINEL_RELEASE_ID --results /secure/local/deployment-command-results.json --strict` to verify every required command result is recorded before `--confirm-import`; each result must match the same release id and hosted URL, point at the expected private artifact path, and include a SHA-256 checksum. The importer rejects missing or mismatched release ids, hosted URLs, deployment-packet metadata, failed release-integrity status, stale checklist entries, missing command-results template lineage, or a missing/blocked deployment execution checklist before posting.
...
- `GOOGLE_OAUTH_REQUESTED_SCOPES`: first-pilot metadata-only scope set. Keep it limited to `drive.metadata.readonly` and `gmail.metadata` unless OAuth review and customer consent justify a broader release.
- `GOOGLE_OAUTH_DEFERRED_RESTRICTED_SCOPES`: restricted remediation scopes documented for later review. The checked-in deployment contract keeps `https://www.googleapis.com/auth/drive` deferred rather than requested.
- `GOOGLE_OAUTH_SCOPE_REVIEW_CONFIRMED`: manual review flag; set to `true` only after the OAuth consent screen, pilot consent packet, and production deployment values all show the same minimal scope boundary.
...
`GET /api/production/provisioning` returns the non-secret provisioning pack for operators: required Google APIs, service accounts, IAM roles, Secret Manager names, private evidence bucket setup, Artifact Registry build command, Pub/Sub setup, render-values audit command, Cloud Run dry-run preflight and packet-verifier commands, Cloud Run dry-run/deploy commands, a redacted deployment transcript collector, release-bound hosted proof collection/import commands, the deployment execution checklist gate, and post-deploy verification sequence. The pack builds the container with the configured Cloud Run region and a tag derived from `SENTINEL_RELEASE_ID`, not `latest`, so source, image, manifest, and revision evidence can be tied to one release. The pack requires the values audit before the manifest render, then requires the preflight packet and digest-verifier JSON before the real `gcloud` dry-run, requires a redacted checksummed dry-run/deploy/describe packet before hosted verification, and routes final Evidence Vault writes through `npm run collect:hosted-proof`, `npm run import:hosted-proof --dry-run`, and `npm run prepare:deployment-execution-checklist` so release-integrity, proof-flag, and command-result checks are not bypassed. It intentionally marks human attestations as manual review and never includes admin tokens, API key values, OAuth client secrets, evidence-signing secrets, Drive channel tokens, judge credentials, invoices, or customer findings.
...
`GET /api/production/deployment-packet` returns the release deployment evidence packet. It binds `SENTINEL_RELEASE_ID`, `NEXT_PUBLIC_PRODUCT_URL`, source repository URL, private evidence bucket, Cloud Run manifest status, local quality gates, the render-values audit, dry-run/deploy/describe commands, hosted `verify:production` commands, hosted evidence capture, release-bound hosted proof bundle artifacts, Evidence Vault import dry-run/checklist/confirm commands, source-release/provenance reports, redaction rules, and an import-template payload into one operator plan. It also includes a private deployment runbook with five stop-gated phases: local preflight, manifest render, Cloud Run dry-run/deploy, hosted production proof, and redacted Evidence Vault import. Each phase lists command ids, required proof files, stop conditions, and redaction checks. It remains `template-needs-values` until release id, product URL, private evidence bucket, admin-token secret, render-values audit, and rendered Cloud Run placeholders are ready; it remains only a plan until the hosted commands are actually executed and checksummed artifacts are registered.
...
Run `npm run verify:cloudrun-deployment` before any Cloud Run dry-run. The verifier requires the checked-in manifest to keep secret values in Secret Manager references with explicit numeric versions and blocks duplicate env names, raw secret-shaped values in any env field, prohibited credential env vars such as `GOOGLE_CLOUD_ACCESS_TOKEN`, `GOOGLE_APPLICATION_CREDENTIALS`, Workspace refresh tokens, and judge passwords, invalid production mode flags, non-HTTPS product/OAuth/PubSub URLs, non-YouTube/Vimeo/Youku demo-video hosts, invalid `XPRIZE_ENTRANT_TYPE`, malformed Google OAuth client ids, restricted or content-access OAuth scopes in the first-pilot requested scope set, missing deferred restricted Drive mutation scope documentation, wildcard or invalid Gemini server IP allowlists, non-positive Gemini budget/quota/cost values, mismatched OAuth redirect or Pub/Sub audience paths, project/billing/resource mismatches, category drift away from Small Business Services, contradictory proof flags such as evidence-response readiness without judge/demo/IP-review flags, and a `GEMINI_MODEL` outside `SENTINEL_GEMINI_MODEL_ALLOWLIST`; repository-access, Google Cloud product, deployed Gemini API-call, business-model proof, category-impact proof, AI-native operations proof, OAuth scope review, IP ownership review, evidence-response readiness, revenue, cost, CAC, user, testimonial-consent, related-party, product-running, agent-log, and quota-evidence flags stay surfaced as manual-review items even when set to `true`, because a deployment manifest can state an attestation but cannot prove the private evidence. Cloud Run production should use the configured runtime service account plus IAM, while local REST smoke tests may still use `GOOGLE_CLOUD_ACCESS_TOKEN` from a private shell.

The checked-in `.env.example` mirrors every non-secret key in the Cloud Run deployment contract so local operators see the same XPRIZE, Gemini, OAuth, Workspace, cost-control, demo, judge-access, and evidence switches before creating private production values. Tests also keep an independent deployment-capability matrix over the manifest and contract, so removing an entire evidence category from both files fails before source can be shipped. Secret fields in `.env.example` stay empty; production secrets belong in Secret Manager references in the rendered Cloud Run manifest.

Run `npm run write:cloudrun-values-template -- /secure/local/cloudrun-render-values.json` to create a private starting file from the tracked non-secret template at `docs/deployment/cloudrun-render-values.template.json`. Fill only non-secret production values: project id, project number, release id, source commit metadata, product URL, billing budget id or short id, OAuth client id, Gemini API key resource id or short id, static egress IPs, entrant type, category, reviewed business-evidence flags, and numeric Secret Manager versions. Then run `npm run audit:cloudrun-values -- --values /secure/local/cloudrun-render-values.json --out-dir artifacts/deployment --release-id RELEASE_ID --strict` to write `cloudrun-render-values-audit.json` and `.md` under `artifacts/deployment/RELEASE_ID/`. The audit fails in strict mode if required non-secret values are missing or placeholder-shaped, if `--release-id` is still placeholder-shaped, or if `--release-id` does not match `SENTINEL_RELEASE_ID` in the private values file. It lists derived values and Secret Manager version references, keeps manual XPRIZE/evidence flags visible, and reminds operators that the filled values file stays private.

After the values audit is `ready-to-render`, run `npm run render:cloudrun-manifest -- --values /secure/local/cloudrun-render-values.json --out-dir artifacts/deployment --release-id RELEASE_ID --strict` to create an ignored private render bundle under `artifacts/deployment/RELEASE_ID/`. Strict mode fails before rendering when required values are absent or still placeholder-shaped, and rendering also fails if the CLI release id and values-file release id disagree. The renderer writes `cloudrun.service.rendered.yaml`, `cloudrun-manifest-verifier.json`, `cloudrun-render-summary.json`, and dry-run/deploy command files; it replaces the Cloud Run secret annotation project-number placeholders and rejects raw API keys, OAuth secrets, refresh tokens, service-account key paths, access tokens, and judge credentials. A `ready-to-dry-run` render is still private deployment-template evidence, not hosted proof.

Run `npm run prepare:cloudrun-dry-run -- --values /secure/local/cloudrun-render-values.json --out-dir artifacts/deployment --release-id RELEASE_ID --strict` immediately before the real Cloud Run dry-run. The preflight command re-renders the private manifest, reruns the verifier, writes `cloudrun-dry-run-preflight-packet.json` and `.md` beside the rendered manifest, and exits non-zero in strict mode unless the packet status is `ready-to-dry-run`. The packet lists stop conditions, files to preserve, SHA-256 digests for the rendered manifest bundle, manual-review evidence flags, and redaction checks for Cloud Run dry-run/deploy output. Immediately before running `gcloud`, run `npm run verify:cloudrun-dry-run-packet -- artifacts/deployment/RELEASE_ID/cloudrun-dry-run-preflight-packet.json --strict`; it writes `cloudrun-dry-run-packet-verifier.json` beside the packet, recomputes the bundle digests, and blocks dry-run if the rendered manifest, verifier, summary, or command files changed after preflight. Preserve that verifier JSON in the private evidence store. These commands do not deploy Cloud Run or prove hosted readiness.

After the real Cloud Run dry-run, deployment, and `gcloud run services describe ... --format=json`, run `npm run collect:cloudrun-deployment -- --release-id RELEASE_ID --dry-run-log /secure/local/cloudrun-dry-run.log --deploy-log /secure/local/cloudrun-deploy.log --describe-json /secure/local/cloudrun-describe.json --out-dir artifacts/deployment --strict`. The collector writes `cloudrun-deployment-transcript-packet.json` and `.md`, embeds redacted copies of the saved logs, computes SHA-256 hashes for the raw and redacted inputs, extracts the deployed service URL, revision, service account, image, env names, Secret Manager env names, and `SENTINEL_RELEASE_ID` from describe JSON, and blocks strict mode if required files are missing, fatal-looking `gcloud` output is present, the deployed env set no longer matches the Cloud Run contract, required secret env vars are not Secret Manager refs, the deployed release id does not match the collected release, the image is not release-bound, or the runtime service account is not the dedicated Sentinel account. It does not run `gcloud`, deploy Cloud Run, call Gemini, or prove hosted readiness by itself.

Run `npm run collect:hosted-proof -- --url https://YOUR-CLOUD-RUN-URL --release-id RELEASE_ID` to write a local hosted proof bundle under `artifacts/hosted-proof/RELEASE_ID/`. The bundle includes `verify-production.json`, hosted launch readiness, deployment packet, hosted evidence, judge access pack, source-release guard, project provenance, license manifest, submission binder, Devpost pack, demo-video pack, evidence-intake queue, Workspace sync status, Claim Guard report, `release-evidence-manifest.json`, `manifest.json`, and a Markdown summary. The collector also indexes the expected private command-results template and deployment execution checklist as release evidence, but marks them missing until the operator generates and fills them outside source. The collector records release-integrity checks so `verify-production.json`, the deployment packet, the Evidence Vault import template, hosted URL, and pushed-source provenance all point at the same release before import. The release manifest groups Cloud Run, production readiness, live Gemini, GCP persistence, Workspace watch lifecycle, cost controls, business proof, judge access, deployment execution control, demo video, repository, license/IP, and Devpost/claim-safety slots as verified, needs-review, missing, mock-only, or transport-error. It also cross-checks `XPRIZE_REPOSITORY_ACCESS_CONFIGURED`, `XPRIZE_GOOGLE_CLOUD_PRODUCT_EVIDENCE_CONFIGURED`, and `XPRIZE_GEMINI_API_CALL_EVIDENCE_CONFIGURED`: if hosted launch readiness claims any of these flags true without matching repository, Google Cloud, or provider=gemini-api proof in the bundle, the release manifest becomes blocked and `npm run import:hosted-proof` rejects the bundle. The folder is ignored by Git. The collector defensively redacts common token/secret-shaped fields, but every generated artifact still needs human redaction review before judge sharing or Evidence Vault import. Add `--include-write-checks` only from a private operator shell after the production admin token and private evidence handling are configured.

Run `npm run import:hosted-proof -- --bundle-dir artifacts/hosted-proof/RELEASE_ID --url https://YOUR-CLOUD-RUN-URL --dry-run` to write `evidence-vault-import-request.json` and confirm the redacted request shape without making a network write. Then run `npm run prepare:deployment-execution-checklist -- --bundle-dir artifacts/hosted-proof/RELEASE_ID --write-results-template /secure/local/deployment-command-results.json` to write a private release-bound results template with every required command id, expected artifact path, and checksum placeholder. Fill that file from reviewed operator output and run `npm run prepare:deployment-execution-checklist -- --bundle-dir artifacts/hosted-proof/RELEASE_ID --results /secure/local/deployment-command-results.json --strict` to write `deployment-execution-checklist.json` and `.md`; the filled results file should record each required command id, `status=passed`, matching `releaseId`, matching `sourceUrl`, ISO `recordedAt`, the expected private `evidencePath`, and a SHA-256 `evidenceSha256` for the release. The checklist records `resultsTemplate` lineage so confirm-import can reject hand-written or stale checklists that bypassed the template. After human redaction review, run `SENTINEL_ADMIN_ACTION_TOKEN=... npm run import:hosted-proof -- --bundle-dir artifacts/hosted-proof/RELEASE_ID --url https://YOUR-CLOUD-RUN-URL --confirm-import` from a private operator shell to POST the redacted `verify-production.json` into `/api/evidence/vault/import`; the script stores a redacted import response and summary back into the ignored bundle. Confirm import requires a passed `deployment-execution-checklist.json`, reads the admin token only from `--admin-token-env`, rejects raw token flags, requires hosted HTTPS by default, blocks stale release/URL/evidence-path/checksum entries, and allows `--allow-local` only for local smoke tests.
...
`GET /api/xprize/license-manifest` returns the dependency and third-party API manifest generated from `package.json` and `package-lock.json`. It summarizes direct runtime dependencies, development dependencies, transitive packages, license review status, Google API integrations, disclosure text, blockers, and next actions. It also includes a hash-bound `reviewPacket` with package-file SHA-256 digests, rule traceability, private artifact requirements, approval flag boundaries, and a clearance checklist for package licenses, Google API terms, OAuth consent, demo/screenshot assets, repository licensing, and pre-existing-work disclosure. The scanner separates restricted blockers from license-review and obligation-review packages; for example, optional transitive packages with LGPL-style obligations are routed to human obligation review instead of being treated as automatic replacement blockers. The manifest is submission-support evidence only; set `XPRIZE_THIRD_PARTY_REVIEW_APPROVED=true` and `XPRIZE_IP_OWNERSHIP_REVIEW_APPROVED=true` only after a human owner reviews dependency licenses, notice/distribution obligations, Google API terms, OAuth consent-screen scope wording, and final asset/IP use. A human approval flag does not mark license/IP proof ready while the generated manifest still has blocked restricted-license or unknown-license items.
