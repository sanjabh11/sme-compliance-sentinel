...
27. Run **Cloud Run evidence** or `npm run verify:cloudrun-deployment` to confirm the checked-in manifest still has only template replacement gaps, Secret Manager references, and manual attestation flags, then run `npm run render:cloudrun-manifest -- --values /secure/local/cloudrun-render-values.json --out-dir artifacts/deployment --release-id $SENTINEL_RELEASE_ID --strict` before a real dry-run.
...
30. Run `npm run collect:hosted-proof -- --url $NEXT_PUBLIC_PRODUCT_URL --release-id $SENTINEL_RELEASE_ID` after deployment to capture an ignored local bundle of redacted hosted proof JSON and `release-evidence-manifest.json`, then run `npm run import:hosted-proof -- --bundle-dir artifacts/hosted-proof/$SENTINEL_RELEASE_ID --url $NEXT_PUBLIC_PRODUCT_URL --dry-run` to preview the Evidence Vault import request before posting it.
...
`GET /api/xprize/provenance` returns the current project-provenance report from the local Git worktree when Git is available. It checks whether commits exist, whether the first commit is after the hackathon start reference, whether source files are tracked, whether a repository URL is configured or detectable from `origin`, whether the local HEAD is present on the upstream branch, and whether the human attestation flag is still missing. `npm run verify:provenance` emits a terminal JSON report for the same evidence class. A branch with no commits, untracked source files, or an unpushed HEAD remains blocked until the source is committed, pushed/shared, and human-reviewed. The current public source repository is `https://github.com/sanjabh11/sme-compliance-sentinel`.
