...
function writeJson(path, value) {
  ...
  assertDirectoryPathSafe(parentDirectory, "Gemini model readiness output parent directory");
  mkdirSync(parentDirectory, { recursive: true });
  assertDirectoryExistsSafe(parentDirectory, "Gemini model readiness output parent directory");
  writeTextFile(absolutePath, `${JSON.stringify(value, null, 2)}\n`, "Gemini model readiness output file");
}
...
function assertSameDirectoryIdentity(path, expected, label) {
  ...
  if (actual.dev !== expected.dev || actual.ino !== expected.ino) {
    throw new Error(`${label} ${resolve(path)} changed while writing; regenerate the private Gemini model readiness packet in a stable private directory.`);
  }
}
